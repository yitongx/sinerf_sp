...
class SirenNeRF(nn.Module):
    def __init__(self, pos_in_dims=3, 
                 ...
                 den_mag=25.0):
        """
        Replace ReLU with SIREN activation. Keep the same architecture of OfficialNerf.
        ...
        ### add one more layer for density output
        # self.fc_density = nn.Sequential(nn.Linear(D, D//2), nn.Linear(D//2, 1))  

        ### A. follow fc_density
        _fc_density = [SirenLinear(D, D // 2, True, 1.), nn.Linear(D // 2, 1, True)] 
        # ### B. cancel SIREN for density-FC
        # _fc_density = [nn.Linear(D, 1, True)]     
        self.fc_density = nn.Sequential(*_fc_density)

        ### A. ReLU fc_feature as original MLP
        self.fc_feature = nn.Linear(D, D)
        ### B. SIREN fc_feature
        # self.fc_feature = SirenLinear(D, D, True, 1.)

        ### A. follow rgb_layers
        _rgb_layers = [SirenLinear(D + dir_in_dims, D // 2, True, 1.), nn.Linear(D // 2, 3, True)] 
        # ### B. cancel SIREN for rgb-FC
        # _rgb_layers = [nn.Linear(D + dir_in_dims, D // 2, True), nn.Linear(D // 2, 3, True)] 
    ...
    def forward(self, pos_enc, dir_enc):
        ...
        # density = torch.where(density > 0, self.den_mag * density, density)
        # density = torch.relu(self.den_mag * density)
