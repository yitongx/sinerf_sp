'''
3.24 Helper file for train_eval_sirennerf
only provide function
'''
import sys
import os
import argparse
...
from tasks.nerfmm.train_eval_siren_ROI import model_render_image, eval_one_epoch_traj
...
def parse_args():
    ...
    parser.add_argument('--opt_eval_lr', default=0.0005, type=float)
    ...
    parser.add_argument('--model_type', type=str, default='sirennerf', help="OfficialNerf or SirenNeRF or sirennerfws")
...
def eval_one_epoch_img(scene_eval, model, focal_net, eval_pose_param_net, my_devices, args, logger):
    ...
    for i in tqdm(range(N_img)):
        ...
        tqdm.write('{0:4d} img: PSNR: {1:.4f}, SSIM: {2:.4f}, LPIPS {3:.4f}'.format(i, psnr, ssim, lpips_loss))
        logger.info('{0:4d} img: PSNR: {1:.4f}, SSIM: {2:.4f}, LPIPS {3:.4f}'.format(i, psnr, ssim, lpips_loss))
    ...
    print('--------------------------')
    print('Mean MSE: {0:.4f} \nPSNR:\n {1:.4f} \nSSIM:\n {2:.4f} \nLPIPS:\n {3:.4f}'.format(mean_mse, mean_psnr,
                                                                                  mean_ssim, mean_lpips))
    logger.info('--------------------------')
    logger.info('Mean MSE: {0:.4f} \nPSNR:\n {1:.4f} \nSSIM:\n {2:.4f} \nLPIPS:\n {3:.4f}'.format(mean_mse, mean_psnr, 
                                                                                                  mean_ssim, mean_lpips))
...
def opt_eval_pose_one_epoch(args, model, focal_net, eval_pose_param_net, scene_eval, optimizer_eval_pose, my_devices):
    '''
    Add "args"
    '''
    model.eval()
...
def eval_main(args):
    ...
    ### print intrinsics
    print('Intrinsic: H: {0:4d}, W: {1:4d}, GT focal {2:.4f}.'.format(scene_train.H, scene_train.W, scene_train.focal))
    logger.info('Intrinsic: H: {0:4d}, W: {1:4d}, GT focal {2:.4f}.'.format(scene_train.H, scene_train.W, scene_train.focal))
    ...
    if args.type_to_eval == 'train':
        ...
        scene_eval = DataLoaderWithCOLMAP(base_dir=args.base_dir,
                                          ...
                                          skip=args.eval_skip,
                                          use_ndc=args.use_ndc)
    print("scene_eval.N_imgs:", scene_eval.N_imgs)
    ...
    elif args.model_type.lower() == 'sirennerfws':
        pos_enc_in_dims = 3
        if args.use_dir_enc:
            dir_enc_in_dims = 3
        else:
            dir_enc_in_dims = 0
        model = SirenNeRF_WS(pos_enc_in_dims, 
                            dir_enc_in_dims, 
                            256, 
                            args.sine_weights_layers0, 
                            args.rgb_mag,
    ...
    if args.init_focal_from == 'colmap':
        focal_net = LearnFocal(scene_train.H, scene_train.W, args.learn_focal, 
                               args.fx_only, order=args.focal_order, init_focal=scene_train.focal)
    else:   # args.init_focal_from == 'none':
        focal_net = LearnFocal(scene_train.H, scene_train.W, args.learn_focal, 
                               args.fx_only, order=args.focal_order, init_focal=None)
    if args.multi_gpu:
    ...
    if 'blender/' in args.scene_name:
        print('GT: fx {0:.4f} fy {1:.4f}, learned: fx {2:.4f}, fy {3:.4f}, COLMAP: {4:.4f}'.format(
            scene_train.gt_fx, scene_train.gt_fy, fxfy[0].item(), fxfy[1].item(), scene_train.focal))
        logger.info('GT: fx {0:.4f} fy {1:.4f}, learned: fx {2:.4f}, fy {3:.4f}, COLMAP: {4:.4f}'.format(
            scene_train.gt_fx, scene_train.gt_fy, fxfy[0].item(), fxfy[1].item(), scene_train.focal))
    else:
        print('COLMAP: {0:.4f}, learned: fx {1:.4f}, fy {2:.4f}, '.format(scene_train.focal, fxfy[0].item(), fxfy[1].item()))
        logger.info('COLMAP: {0:.4f}, learned: fx {1:.4f}, fy {2:.4f}, '.format(scene_train.focal, fxfy[0].item(), fxfy[1].item()))
    ...
    print("=====> Loading learned_pose_param_net ckpt")
    learned_pose_param_net = load_ckpt_to_net(os.path.join(args.ckpt_dir, 'latest_pose.pth'), learned_pose_param_net, map_location=my_devices)
    ...
    else:   # args.type_to_eval == 'val':
        with torch.no_grad():
            # compuate a scale between two learned traj and colmap traj
            # then rescale eval colmap traj with the nerfmm traj
            learned_c2ws_train = torch.stack([learned_pose_param_net(i) for i in range(scene_train.N_imgs)])  # (N, 4, 4)
            learned_c2ws_train = learned_c2ws_train.to(device=my_devices)
            
            colmap_c2ws_train = scene_train.c2ws  # (N, 4, 4)
            init_c2ws = scene_eval.c2ws # To be aligned
            init_c2ws = init_c2ws.to(device=my_devices)

            ### 5.25 Align Evaluation Poses with SIM(3) from learned training poses and COLMAP training poses.
            init_c2ws, scale_colmap2est = align_scale_c2b_use_a2b(colmap_c2ws_train, learned_c2ws_train, init_c2ws)

        ### init eval_pose_param_net with aligned init_c2ws
    ...
    '''Optimise eval poses'''
    # if args.type_to_eval != 'train':
    if args.type_to_eval == 'val':
        for epoch_i in tqdm(range(args.opt_pose_epoch), desc='optimising eval'):
            # mean_losses = opt_eval_pose_one_epoch(model, focal_net, eval_pose_param_net, scene_eval, optimizer_eval_pose, my_devices)
            mean_losses = opt_eval_pose_one_epoch(args, model, focal_net, eval_pose_param_net, scene_eval, optimizer_eval_pose, my_devices)
    ...
    with torch.no_grad():
        ...
        print('------------------ ATE statistic ------------------')
        logger.info('Traj Err: translation: {0:.6f}, rotation: {1:.6f} deg, scale: {2:.6f}'.format(stats_tran['mean'], stats_rot['mean'], stats_scale['mean']))
        tqdm.write('Traj Err: translation: {0:.6f}, rotation: {1:.6f} deg, scale: {2:.6f}'.format(stats_tran['mean'], stats_rot['mean'], stats_scale['mean']))
        print('-------------------------------------------------')
        ...
        depths = (depths.cpu().numpy() * 255).astype(np.uint8)
