# 5.26 for presentation's visualization!
import sys
import os
import argparse
...
from models.nerf_models import OfficialNerf, SirenNeRF
# from tasks.nerfmm.train import model_render_image
from tasks.nerfmm.train_sirennerf import model_render_image
...
def test_one_epoch(H, W, focal_net, c2ws, near, far, model, my_devices, args, img_out_dir, depth_out_dir):
    model.eval()
    focal_net.eval()
    ...
    for i in tqdm(range(N_img)):
        ...
        depth = (rendered_depth.cpu().numpy() * 255).astype(np.uint8)
        imageio.imwrite(os.path.join(img_out_dir, str(i).zfill(4) + '.png'), img)
        imageio.imwrite(os.path.join(depth_out_dir, str(i).zfill(4) + '.png'), depth)
        print("=> Saved on image {}/{}".format(i, N_img-1))
...
def main(args):
    ...
    '''Create Folders'''
    test_dir = Path(os.path.join(args.ckpt_dir, 'spiral'))
    ...
    '''Render'''
    result = test_one_epoch(H, W, focal_net, c2ws, near, far, model, my_devices, args, img_out_dir, depth_out_dir)
    imgs = result['imgs']
