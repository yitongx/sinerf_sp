...
train_sirennerf + eval
add cyclic LR schedule
MRS
'''
import enum
...
from models.nerf_models import OfficialNerf, SirenNeRF, SirenNeRF_WS
...
def parse_args():
    ...
    ### CyclicLR
    parser.add_argument('--use_nerf_cyclic', default=False, action='store_true')
    parser.add_argument('--max_nerf_lr', default=0.005, type=float)
    parser.add_argument('--nerf_size_up', default=50, type=int)
    
    parser.add_argument('--use_focal_cyclic',  default=False, action='store_true')
    parser.add_argument('--max_focal_lr', default=0.005, type=float)
    parser.add_argument('--focal_size_up', default=50, type=int)

    parser.add_argument('--use_pose_cyclic', default=False, action='store_true')
    parser.add_argument('--max_pose_lr', default=0.005, type=float)
    parser.add_argument('--pose_size_up', default=50, type=int)
    ...
    parser.add_argument('--ROI_schedule_tail', type=float, default=0.3)

    parser.add_argument('--perturb_t', type=bool, default=True)
    parser.add_argument('--sigma_noise', type=float, default=0.0, help="Default no sigma noise. Otherwise may harm scenes like Horns.")
    ...
    parser.add_argument('--den_mag', type=float, default=25.0)
    parser.add_argument('--rgb_mag', type=float, default=1.0)
    

    parser.add_argument('--model_type', type=str, default='SirenNeRF', help="OfficialNerf or SirenNeRF")
    parser.add_argument('--alias', type=str, default='', help="experiments alias")
    parser.add_argument('--detail_name', type=str, default='', help="detail_name")
    # parser.add_argument('--experiment_dir', type=str, default='', help="for globally share experiment_dir")
...
def gen_detail_name(args):
    if args.model_type.lower() == 'officialnerf':
        m_type = 'official'
        ...
        if args.use_nerf_cyclic:
            outstr += '_nerf_lr' + "_[{}|{}|{}|{}]".format(str(args.nerf_lr), str(args.max_nerf_lr), str(args.nerf_lr_gamma), str(args.nerf_size_up))
        else:
            outstr += '_nerf_lr' + "_{}".format(args.nerf_lr)

        if args.use_focal_cyclic:
            outstr += '_focal_lr' + "_[{}|{}|{}|{}]".format(str(args.focal_lr), str(args.max_focal_lr), str(args.focal_lr_gamma), str(args.focal_size_up))
        else:
            outstr += '_focal_lr' + "_{}".format(args.focal_lr)
        if args.use_pose_cyclic:
            outstr += '_pose_lr' + "_[{}|{}|{}|{}]".format(str(args.pose_lr), str(args.max_pose_lr), str(args.pose_lr_gamma), str(args.pose_size_up))
        else:
            outstr += '_pose_lr' + "_{}".format(args.pose_lr)
        ...
        # outstr = str(datetime.datetime.now().strftime('%m%d_%H%M')) + \
        #         '_' + m_type + \
        #         '_lr' + str(args.nerf_lr) + \
        #         '_resize_' + str(args.resize_ratio) + \
        #         '_seed_' + str(args.rand_seed) + \
        #         '_' + str(args.alias)
                
                
    elif args.model_type.lower() == 'sirennerf':
        m_type = 'sirennerf'
        outstr = str(datetime.datetime.now().strftime('%m%d_%H%M')) + \
                '_' + m_type
...
def model_render_image(c2w, rays_cam, t_vals, 
    ...
    if args.model_type.lower() == 'officialnerf':
        ...
        # pos_enc = encode_position(sample_pos, levels=0, inc_input=True)
    ...
    if args.use_dir_enc:
        ...
        if args.model_type.lower() == 'officialnerf':
            ...
            # dir_enc = encode_position(ray_dir_world, levels=0, inc_input=True)  # (H, W, C*(2*L+1))
...
def train_one_epoch(scene_train, 
    ...
    for i in ids:
        ...
        '''
        - Turn on perturb_t for training
        - Set sigma_noise to 1.0
        '''
        ...
        if args.use_ROI and epoch_i / args.epoch >= args.ROI_schedule_head and epoch_i / args.epoch < args.ROI_schedule_tail:
            ...
            # print("rc_ids_roi: ", rc_ids_roi.shape)
            ...
            # print("r_id_roi: ", r_id_roi.shape, "r_id_rand: ",  r_id_rand.shape)
            ...
            # print("ray_selected_cam: ", ray_selected_cam.shape, "img_selected: ",img_selected.shape)
        else:
...
def main(args):
    ...
    if args.use_ROI:
        print("===> len(interest_indices): {}, {}".format(len(scene_train.interest_indices), 
                                                        scene_train.interest_indices[0].shape))
    ...
    if args.model_type.lower() == 'officialnerf':
        ...
        if args.use_dir_enc:
            ...
            dir_enc_in_dims = 0
        model = SirenNeRF(pos_enc_in_dims, 
                        dir_enc_in_dims, 
                        args.hidden_dims, 
    ...
    if args.start_refine_focal_epoch > -1:
        ...
        focal_net = LearnFocal(scene_train.H, scene_train.W, args.learn_focal, 
                               args.fx_only, order=args.focal_order)
    if args.multi_gpu:
    ...
    if not args.use_nerf_cyclic:
        scheduler_nerf = torch.optim.lr_scheduler.MultiStepLR(optimizer_nerf, milestones=args.nerf_milestones, gamma=args.nerf_lr_gamma)
    else:
        scheduler_nerf = torch.optim.lr_scheduler.CyclicLR(optimizer_nerf, base_lr=args.nerf_lr, 
                                                           max_lr=args.max_nerf_lr, step_size_up=args.nerf_size_up, 
                                                           mode='exp_range', gamma=args.nerf_lr_gamma, cycle_momentum=False)
    if not args.use_focal_cyclic:
        scheduler_focal = torch.optim.lr_scheduler.MultiStepLR(optimizer_focal, milestones=args.focal_milestones, gamma=args.focal_lr_gamma)
    else:
        scheduler_focal = torch.optim.lr_scheduler.CyclicLR(optimizer_focal, base_lr=args.focal_lr,
                                                            max_lr=args.max_focal_lr, step_size_up=args.focal_size_up, 
                                                            mode='exp_range', gamma=args.focal_lr_gamma, cycle_momentum=False)

    if not args.use_pose_cyclic:
        scheduler_pose = torch.optim.lr_scheduler.MultiStepLR(optimizer_pose, milestones=args.pose_milestones, gamma=args.pose_lr_gamma)
    else:
        scheduler_pose = torch.optim.lr_scheduler.CyclicLR(optimizer_pose, base_lr=args.pose_lr, 
                                                           max_lr=args.max_pose_lr, step_size_up=args.pose_size_up, 
                                                           mode='exp_range', gamma=args.pose_lr_gamma, cycle_momentum=False)
